                                ...
                                <div class="row">
                                    ...
                                    <div class="col">
                                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                            <a href="{{ route('rekap.pengaduan') }}" class="btn btn-md btn-success mb-3"
                                                style="background-color: #8C0C14; border:none">Rekapitulasi Pengaduan</a>
                                        </div>
                                    </div>
                                </div>
                                <table id="example" class="table" style="width:100%;">
                                    ...
                                    <tbody class="table">
                                        @php
                                            $no = 1;
                                        @endphp
                                        @foreach ($admin_pengaduan as $item)
                                            @if ($item->is_deleted == 'no' && $item->user_id)
                                                <tr>
                                                    <td>{{ $no++ }}</td>
                                                    ...
                                                    <td class="text-center">
                                                        <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                                                            action="{{ route('destroy.pengaduan.admin', $item->id) }}"
                                                            method="GET">
                                                            @if ($item->status != 'Terkirim')
                                                                <a href="{{ route('show.balaspengaduan.admin', $item->id) }}"
                                                                    class="btn btn-sm btn-dark m-1" role="button"
                                                                    title="Lihat detail pengaduan" style="width: 75px">
                                                                    Lihat
                                                                </a>
                                                            @else
                                                                <a href="{{ route('show.balaspengaduan.status', $item->id) }}"
                                                            ...
                                                            @if ($item->status == 'Terkirim')
                                                                <button type="submit" class="btn btn-sm btn-danger m-1"
                                                                    title="Hapus pengaduan" style="width: 75px">
                                                                    Hapus
                                                                </button>
                                                            @else
                                                                <button type="submit"
